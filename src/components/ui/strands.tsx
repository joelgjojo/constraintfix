...
export function Strands({ active = true, className = "" }: StrandsProps) {
  return (
    <svg aria-hidden="true" className={`strands ${active ? "strands--active" : ""} ${className}`} viewBox="0 0 600 160" preserveAspectRatio="none">
      <defs>
        <linearGradient id="constraintfix-strand-a" x1="0" x2="1">
          <stop stopColor="#60A5FA" stopOpacity="0" />
          <stop offset="0.42" stopColor="#60A5FA" />
          <stop offset="0.7" stopColor="#38BDF8" />
          <stop offset="1" stopColor="#60A5FA" stopOpacity="0" />
        </linearGradient>
        <filter id="constraintfix-strand-blur"><feGaussianBlur stdDeviation="2" /></filter>
      </defs>
      <g fill="none" stroke="url(#constraintfix-strand-a)" strokeLinecap="round">
        <path className="strands__glow" d="M-20 112 C120 20 210 176 350 69 S540 66 620 34" filter="url(#constraintfix-strand-blur)" />
