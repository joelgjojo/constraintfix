...
/** Keeps an explicitly requested live demo moving if OpenAI cannot return a valid candidate. */
  ...
  return {
    ...
    async decide(input: AgentDecisionInput): Promise<RepairDecision> {
      try {
        ...
        return {
          ...decision,
          fallbackNotice: "OpenAI Live was unavailable or returned an invalid candidate, so ConstraintFix continued with the verified replay response.",
        };
      }
    },
  };
